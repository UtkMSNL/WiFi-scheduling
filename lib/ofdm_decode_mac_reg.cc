 ...
 * 	The module name is based on the orignal code "ofdm_decode_mac"
 *
...
class ofdm_decode_mac_reg_impl : public ofdm_decode_mac_reg {

public:
ofdm_decode_mac_reg_impl(bool log, bool debug) : block("ofdm_decode_mac_reg",
...
void decode() {
	...
	print_output();
 	...
 	// The following codes are commented by Haoyang
/*	// skip service field
...
*/
 	// The following codes are added by Haoyang
	if(print_info() == 0) {
		// skip service field
		boost::crc_32_type result;
		result.process_bytes(out_bytes + 2, d_tx.psdu_size);
...
private:
	...
	/* The following two lines are added by Haoyang */
...
ofdm_decode_mac_reg::sptr
ofdm_decode_mac_reg::make(bool log, bool debug) {
	return gnuradio::get_initial_sptr(new ofdm_decode_mac_reg_impl(log, debug));
}
