 ...
 *
 * Log:
 * 	2016-01-30: Only receive VLC frames which set the BIT4 (RESERVED) to 1
 *  			to indicate the vlc frame type
 */
...
bool print_signal() {
	...
	// ADDED by Haoyang, Only receive VLC frames which set the reserved bit (bit 4) to 1
	if((bool)decoded_bits[4]) {
		dout << "VLC frame received" << std::endl;
		d_copy_symbols = 14;
		return true;
	}
	else {
